...
export default function DatePillPicker({ value, onChange, right }: Props) {
  ...
  return (
    <div className="relative " ref={ref}>
      <div
        onClick={() => setOpen((p) => !p)}
        className="h-12 px-4 gap-2 flex items-center min-w-28 rounded-full border border-gray-600 cursor-pointer hover:bg-gray-50"
      >
        <p className="text-2xl lg:text-3xl leading-none">{hookDate}</p>
        <div className="text-[10px] lg:text-xs leading-tight">
          <p>{month}</p>
          <p>{year}</p>
        </div>
      </div>
