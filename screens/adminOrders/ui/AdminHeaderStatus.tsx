...
const OPTIONS = [
  ...
  { label: 'Preparing', value: 'PREPARING', color: 'text-blue-500' },
  ...
  return (
    <div className="relative z-50">
      ...
      <div className={`absolute bg-white right-0 w-full  xl:w-40 shadow-md top-[110%] duration-300 border-2 border-gray-200  overflow-hidden rounded-xl gap-3 ${open ? "translate-y-0 opacity-100 visible" : "opacity-0 translate-y-3 invisible"}`}>
        {OPTIONS.map((el) => (
          <div key={el.value} className={`text-[13px]  cursor-pointer duration-300 font-medium  ${el.value == orderStatus ? "bg-blue-600 text-white border-blue-600" : "bg-white hover:bg-blue-100 border-gray-200"} px-4 py-2 `} onClick={() => setOrderStatus(el.value)}>
            <p>{el.label}</p>
          </div>
        ))}
      </div>
