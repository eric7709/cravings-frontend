...
export default function CartFooter() {
    ...
    return (
        <div className={`flex w-full py-3 bg-white/0 border-t border-gray-200 duration-300 backdrop-blur items-center gap-5 justify-between px-4 ${getTotal() > 0 ? "opacity-100 visible" : "opacity-0 invisible"}`}>
            ...
            <button className={`px-6 py-3 font-semibold  text-sm text-white rounded-xl border-2 cursor-pointer duration-300  ${unavailableError ? "bg-gray-300" : "shadow-md active:scale-90 bg-orange-500 hover:bg-blue-700 shadow-gray-400"}`} onClick={book} disabled={unavailableError}>Check out</button>
        </div>
    )
}
